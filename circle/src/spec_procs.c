...
SPECIAL(guild)
{
   ...
   case CLASS_MAGIC_USER :
	 ...
	 return(TRUE);
      ...
      percent = SKILL(ch, number) + MAX(25, int_app[GET_INT(ch)].learn);
      SKILL(ch, number) = MIN(95, percent);
   ...
   case CLASS_THIEF:
	 ...
	 for (i = 0; *t_skills[i] != '\n'; i++) {
	    sprintf(buf2, "%-20s %s\n\r", t_skills[i],
	        how_good(SKILL(ch, i + SKILL_SNEAK)));
	    strcat(buf, buf2);
	 }
	 send_to_char(buf, ch);
	 return(TRUE);
	 ...
	 return(TRUE);
      ...
      if (SKILL(ch, number + SKILL_SNEAK) >= 85) {
	 send_to_char("You are already learned in this area.\n\r", ch);
	 return(TRUE);
      }
      send_to_char("You Practice for a while...\n\r", ch);
      ...
      percent = SKILL(ch, number + SKILL_SNEAK) + MIN(int_app[GET_INT(ch)].learn, 12);
      SKILL(ch, number + SKILL_SNEAK) = MIN(85, percent);
   ...
   case CLASS_WARRIOR:
	 ...
	 for (i = 0; *w_skills[i] != '\n'; i++) {
	    sprintf(buf2, "%-20s %s\n\r", w_skills[i],
	        how_good(SKILL(ch, i + SKILL_KICK)));
	    strcat(buf, buf2);
	 }
	 send_to_char(buf, ch);
	 return(TRUE);
	 ...
	 return(TRUE);
      ...
      if (SKILL(ch, number + SKILL_KICK) >= 80) {
	 send_to_char("You are already learned in this area.\n\r", ch);
	 return(TRUE);
      }
      send_to_char("You Practice for a while...\n\r", ch);
      ...
      percent = SKILL(ch, number + SKILL_KICK) + MIN(12, int_app[GET_INT(ch)].learn);
      SKILL(ch, number + SKILL_KICK) = MIN(80, percent);
