...
struct room_data *world = 0;              /* dyn alloc'ed array of rooms     */
...
FILE *help_fl = 0;                    /* file for help texts (HELP <kwd>)*/
FILE *player_fl = 0;		      /* player file */
...
void	zone_update(void)
{
   ...
   for (update_u = reset_q.head; update_u; update_u = update_u->next)
	 ...
	 syslog(buf, CMP, LEVEL_GOD, FALSE);
	 /* dequeue */
   ...
   for (cmd_no = 0; ; cmd_no++) {
	 ...
	 default:
	    sprintf(buf, "SYSERR:  Unknown cmd in reset table; zone %d cmd %d.\n\r",
	        zone_table[zone].number, cmd_no + 1);
	    syslog(buf, NRM, LEVEL_GRGOD, TRUE);
	    break;
	 }
      else
	 last_cmd = 0;
