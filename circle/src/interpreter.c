...
void	assign_command_pointers (void)
{
   ...
   COMMANDO(11 , POSITION_RESTING , do_drink    , 0, 0)
   COMMANDO(12 , POSITION_RESTING , do_eat      , 0, 0)
   ...
   COMMANDO(75 , POSITION_SLEEPING, do_setskill , LEVEL_GRGOD, 0)
   ...
   COMMANDO(88 , POSITION_RESTING , do_sip      , 0, 0)
   COMMANDO(89 , POSITION_RESTING , do_taste    , 0, 0)
...
void	nanny(struct descriptor_data *d, char *arg)
{
   ...
   case CON_PWDNRM:	/* get pwd for known player	*/
	 ...
	 if (GET_LEVEL(d->character) < restricted) {
	    SEND_TO_Q("The game is temporarily restricteded.. try again later.\n\r", d);
	    STATE(d) = CON_CLOSE;
	 ...
	 for (tmp_ch = character_list; tmp_ch; tmp_ch = tmp_ch->next)
	    if (!IS_NPC(tmp_ch) && 
		  ...
		  sprintf(buf, "%s has re-logged in ... disconnecting old socket.",
		      GET_NAME(tmp_ch));
                  syslog(buf, NRM, MAX(LEVEL_IMMORT, GET_INVIS_LEV(tmp_ch)), TRUE);
	 ...
	 sprintf(buf, "%s [%s] has connected.", GET_NAME(d->character), d->host);
         syslog(buf, BRF, MAX(LEVEL_IMMORT, GET_INVIS_LEV(d->character)), TRUE);
   ...
   case CON_QCLASS:
	 ...
	 return;
      ...
      syslog(buf, NRM, LEVEL_IMMORT, TRUE);
      break;
