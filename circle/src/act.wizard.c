...
void do_stat_character(struct char_data *ch, struct char_data *k)
{
   ...
   sprintf(buf2, " %s - Name: %s [IDNum: %-5d] [RNum: %-5d], In room [%-5d]\n\r",
       (!IS_NPC(k) ? "PC " : (!IS_MOB(k) ? "NPC" : "MOB")),
       GET_NAME(k), GET_IDNUM(k), k->nr, world[k->in_room].number);
   strcat(buf, buf2);
   send_to_char(buf, ch);
   if (IS_MOB(k)) {
...
*/
   ...
   sprintf(buf, "AC:[%d/10], Coins: [%d], Bank Account: [%d]\n\rExp: [%d], Hitroll: [%d], Damroll: [%d]\n\r",
       GET_AC(k), GET_GOLD(k), GET_BANK_GOLD(k), GET_EXP(k),
   ...
   sprintf(buf2, ", Idle Timer (in tics) [%d]", k->specials.timer);
   strcat(buf, buf2);
   ...
   sprintf(buf, "Thirst: %d, Hunger: %d, Drunk: %d\n\r",
       GET_COND(k, THIRST), GET_COND(k, FULL), GET_COND(k, DRUNK));
   send_to_char(buf, ch);
...
ACMD(do_shutdown)
{
   extern int	circle_shutdown, reboot;
...
void	do_start(struct char_data *ch)
{
   ...
   send_to_char("Welcome.  This is now your character in CircleMud.\n\r"
		"You can now earn gold, experience points, and lots more...\n\r",
                             ch);
   ...
   case CLASS_THIEF :
      SKILL(ch, SKILL_SNEAK) = 10;
      SKILL(ch, SKILL_HIDE) = 5;
      SKILL(ch, SKILL_STEAL) = 15;
      SKILL(ch, SKILL_BACKSTAB) = 10;
      SKILL(ch, SKILL_PICK_LOCK) = 10;
      break;
...
ACMD(do_last)
{
   ...
   sprintf(buf, "[%5d] [%2d %s] %-12s : %-18s : %-20s\n\r",
       chdata.specials2.idnum, chdata.level, class_abbrevs[chdata.class],
...
ACMD(do_wiznet)
{
   ...
   if (level > LEVEL_IMMORT) {
      sprintf(buf1, "%s: <%d> %s%s%s\n\r", GET_NAME(ch), level, emote ? "<--- " : "",
          argument, emote ? "" : "");
      sprintf(buf2, "Someone: <%d> %s%s%s\n\r", level, emote ? "<--- " : "",
          argument, emote ? "" : "");
   } else {
...
ACMD(do_zreset)
{
   ...
   if (i >= 0 && i <= top_of_zone_table) {
      ...
      syslog(buf, NRM, MAX(LEVEL_GRGOD, GET_INVIS_LEV(ch)), TRUE);
   } else
...
ACMD(do_wizutil)
{
   ...
   case SCMD_PARDON:
	 ...
	 return;
      ...
      syslog(buf, BRF, MAX(LEVEL_GOD, GET_INVIS_LEV(ch)), TRUE);
      break;
   case SCMD_NOTITLE:
   ...
   case 2: /* player */
	 ...
	 return;
      ...
      sprintf(buf, "Player: %-12s (%s) [%2d %s]\n\r", vbuf.name,
          genders[vbuf.sex], vbuf.level, class_abbrevs[vbuf.class]);
      sprintf(buf,
      ...
      sprintf(buf,
          ...
          buf, ctime(&vbuf.birth), ctime(&vbuf.last_logon),
          (int)(vbuf.played / 3600), (int)(vbuf.played / 60 % 60));
      send_to_char(buf, ch);
      break;
   case 3:
